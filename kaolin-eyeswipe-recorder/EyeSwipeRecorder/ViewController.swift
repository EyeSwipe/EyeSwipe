...
class ViewController: UIViewController {
	...
	@IBOutlet fileprivate var preview: UIView!
	@IBOutlet fileprivate var messaging: UILabel!
    
    let serverURL : String = "eyeswipe.ddns.net"
	...
	func activateAudio() {
		do {
			try AVAudioSession.sharedInstance().setActive(true) //Set to false to deactivate session
	...
	func deactivateAudio() {
		do {
			try AVAudioSession.sharedInstance().setActive(false) //Set to false to deactivate session
		} catch let error as NSError {
	...
	func speak(word:String) {
		// there must be a better way to do this but I haven't used swift enough lately
		var letters = ""
	...
	@objc func onEyeswipeSpacebar(_ notification:Notification) {
        if self.cameraController.userEmail == nil {
            self.cameraController.userEmail = userEmail!
        }
        if self.cameraController.serverURL == nil {
            self.cameraController.serverURL = serverURL
        }
        stage = (stage + 1) % 3
        if stage == 0 {
            if isRecording {
                self.view.resignFirstResponder()
                cameraController.completeRecording()
                isRecording = false
            }
            self.messaging.text = "Press space to display a sentence and hear it read aloud. When ready to record, press space again. When finished recording, press space a third time."
            self.messaging.isHidden = false
        }
        if stage == 1 {
            let group = DispatchGroup()
            let url = URL(string: "http://\(serverURL):8080/sentence/get")!
            group.enter()
            var sentence : String?
            ...
            group.notify(queue: .main) {
                ...
                self.speak(word: sentence!)
                self.storedSentence = sentence!
                self.messaging.text = sentence!
            }
		}
        if stage == 2 {
            self.messaging.isHidden = true
            isRecording = true
            self.cameraController.startRecording(word:storedSentence)
		}
	}
	
	@objc func onCancelRecording(_ notification:Notification) {
		if isRecording {
	...
	@objc func onVideoComplete(_ notification:Notification) {
		isRecording = false
		DispatchQueue.main.async {
		self.messaging.isHidden = false
...
extension ViewController {
	...
	override func viewDidLoad() {
		...
		NotificationCenter.default.addObserver(self, selector: #selector(onEyeswipeSpacebar(_:)), name: .eyeswipeOnSpacebar, object: nil)
		NotificationCenter.default.addObserver(self, selector: #selector(onCancelRecording(_:)), name: .eyeswipeCancel, object: nil)
		NotificationCenter.default.addObserver(self, selector: #selector(onVideoComplete(_:)), name: .eyeswipeVideoComplete, object: nil)
